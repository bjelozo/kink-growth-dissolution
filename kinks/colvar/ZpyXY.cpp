...
//+ENDPLUMEDOC
    ...
    public:
      ...
      ZpyXY(const ActionOptions&);    // CONSTRUCTOR
      ~ZpyXY();                       // DESTRUCTOR
      // active methods:
    ...
    PLUMED_REGISTER_ACTION(ZpyXY,"ZPYXY")
    
    void ZpyXY::registerKeywords( Keywords& keys ){

      Colvar::registerKeywords(keys);
      keys.add("atoms", "CENTER", "the labels of the atoms acting as center of the molecules");

      keys.add("compulsory", "ZETAL_X", "lower position of f_x");
      keys.add("compulsory", "ZETAU_X", "upper position of f_x");
      keys.add("compulsory", "SIGMAL_X", "steepness of f_x on lower side");
      keys.add("compulsory", "SIGMAU_X", "steepness of f_x on upper side");

      keys.add("compulsory", "ZETAL_Y", "lower position of f_y");
      keys.add("compulsory", "ZETAU_Y", "upper position of f_y");
      keys.add("compulsory", "SIGMAL_Y", "steepness of f_y on lower side");
      keys.add("compulsory", "SIGMAU_Y", "steepness of f_y on upper side");
    ...
    ZpyXY::ZpyXY(const ActionOptions&ao):
...
void ZpyXY::kernel_lu(double q_i, double zetal, double zetau, double sigmal, double sigmau) {
...
void ZpyXY::layerindices(vector<Vector> &pos, vector<int> &list) {
...
void ZpyXY::calculate()
{
  ...
  for (unsigned int i=rank; i<ll; i+=stride) {  // SUM OVER MOLECULES
    ...
    kernel_lu(pos[i][2], zetal_z, zetau_z, sigmal_z, sigmau_z);  // calculates f_z and df_z
    f_z = f_lu_i;
    df_z = df_lu_i;
    ...
    // for debugging:
    cv_val_i[list[i]] = f_x*f_y*f_z;

    // sum the total derivative in each direction
    unsigned int index_i = list[i];  // true index of molecule i
    deriv[index_i][0] += df_x*f_y*f_z;
    deriv[index_i][1] += f_x*df_y*f_z;
    deriv[index_i][2] += f_x*f_y*df_z;
